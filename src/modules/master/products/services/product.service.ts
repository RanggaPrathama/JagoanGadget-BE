...
export class ProductService {
  ...
  async findAll(
    ...
    const qb = this.joinSkus(this.repo.createQueryBuilder('p'));
  ...
  async findOne(id: string): Promise<ProductEntity> {
    const product = await this.joinSkus(
      this.repo
        .createQueryBuilder('p')
        .leftJoinAndSelect('p.category', 'cat')
        .leftJoinAndSelect('p.brand', 'brand'),
  ...
  private joinSkus(qb: SelectQueryBuilder<ProductEntity>) {
    return qb
